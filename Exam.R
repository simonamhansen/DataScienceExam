...
# Using WAIC
loo_compare(waic(fullrank_loglik), waic(Meanrank_loglik), waic(Sample_loglik))
...
sample_pred = apply(ext_samp$y_pred, c(2,3), median)
meanrank_pred = apply(ext_meanrank$y_pred, c(2,3), median)


plot(x[,2], type = "p") 
#points(sample_pred[,2])
#lines(x[,2], col = "red")
points(sample_pred[,2] , col = "red")

plot(x[,2], type = "p") 
#points(sample_pred[,2])
#lines(x[,2], col = "red")
points(meanrank_pred[,2] , col = "red")
...
p1=ggplot()+geom_density(aes(samp_maps$Arew - vi_maps$Arew, fill = "red")) + theme_minimal()+guides(fill=FALSE) + xlab("Arew") + scale_fill_manual(values = "red")
p2=ggplot()+geom_density(aes(samp_maps$Apun - vi_maps$Apun, fill = "red")) + theme_minimal()+guides(fill=FALSE) + xlab("Apun") + scale_fill_manual(values = "red")
p3= ggplot()+geom_density(aes(samp_maps$K - vi_maps$K, fill = "red")) + theme_minimal()+guides(fill=FALSE) + xlab("K") + scale_fill_manual(values = "red")
p4=ggplot()+geom_density(aes(samp_maps$wf - vi_maps$wf, fill = "red")) + theme_minimal()+guides(fill=FALSE) + xlab("wf") + scale_fill_manual(values = "red")
p5=ggplot()+geom_density(aes(samp_maps$wp - vi_maps$wp, fill = "red")) + theme_minimal()+guides(fill=FALSE) + xlab("wp") + scale_fill_manual(values = "red")
...
p6 = ggplot() +geom_density(aes(samp_hdis$Arew - vi_hdis$Arew, fill = "bla")) + theme_minimal()+ guides(fill=FALSE) + xlab("Arew") + scale_fill_manual(values = "blue")
p7 = ggplot() +geom_density(aes(samp_hdis$Apun - vi_hdis$Apun, fill = "bla")) + theme_minimal()+ guides(fill=FALSE) + xlab("Apun") + scale_fill_manual(values = "blue")
p8 = ggplot() +geom_density(aes(samp_hdis$K - vi_hdis$K, fill = "bla")) + theme_minimal()+ guides(fill=FALSE) + xlab("K") + scale_fill_manual(values = "blue")
p9 = ggplot() +geom_density(aes(samp_hdis$wf - vi_hdis$wf, fill = "bla")) + theme_minimal()+ guides(fill=FALSE) + xlab("wf") + scale_fill_manual(values = "blue")
p10 = ggplot() +geom_density(aes(samp_hdis$wp - vi_hdis$wp, fill = "bla")) + theme_minimal()+ guides(fill=FALSE) + xlab("wp") + scale_fill_manual(values = "blue")
